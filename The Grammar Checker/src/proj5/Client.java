...
//        System.out.println(testWordCounter.toStringByFreq());
...
//        testGrammarChecker.improveGrammar("src/lamb.txt");
        ...
        // only run this client when you have "input.txt"
        // get the frequencies ranking, write to 'ranking.txt'
        WordCounter ranking = new WordCounter();
        ranking.findFrequencies("src/input.txt");
        System.out.println(ranking.toStringByFreq());

        // convert to a new essay as 'output.txt'
        GrammarChecker converted = new GrammarChecker("src/bigThesaurus.txt", 2);
        converted.improveGrammar("src/input.txt");
    }
}
